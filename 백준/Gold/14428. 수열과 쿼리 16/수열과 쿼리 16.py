...
def create_tree(N, values):
  ...
  def make_tree(node, x, y):
    ...
    if values[tree[node * 2]] > values[tree[node * 2 + 1]]:
...
# 구하고자 하는 범위는 고정이며,
# 세그먼트 트리의 범위를 잘게 쪼개가며 최솟값을 구한다.
def query(N, values, tree, x, y):
  def recursion(node, left, right):
    ...
    return q if values[p] > values[q] else p
...
if __name__ == '__main__':
  ...
  for _ in range(M):
    ...
    if C == 2:
      print(query(N, values, tree, X - 1, Y - 1) + 1)
